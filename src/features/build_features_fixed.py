...
class FeatureEngineer:
    ...
    def normalize_timezone(self, df):
        """Convert any timezone-aware index to UTC and then remove timezone"""
    ...
    def load_all_data(self):
        ...
        if silver_files:
            ...
            data['silver'] = df
            logger.info(f"Loaded silver data: {df.shape} from {df.index.min()} to {df.index.max()}")
        ...
        if gold_files:
            ...
            df.columns = [col.lower() for col in df.columns]
            data['gold'] = df
            logger.info(f"Loaded gold data: {df.shape} from {df.index.min()} to {df.index.max()}")
        ...
        if macro_files:
            ...
            df = self.normalize_timezone(df)
            data['macro'] = df
            logger.info(f"Loaded macro data: {df.shape} from {df.index.min()} to {df.index.max()}")
        ...
        if market_files:
            ...
            df = self.normalize_timezone(df)
            data['market'] = df
            logger.info(f"Loaded market features: {df.shape} from {df.index.min()} to {df.index.max()}")
        ...
        if cot_files:
            ...
            df = self.normalize_timezone(df)
            data['cot'] = df
            logger.info(f"Loaded COT data: {df.shape} from {df.index.min()} to {df.index.max()}")
        ...
        if etf_files:
            ...
            df = self.normalize_timezone(df)
            data['etf'] = df
            logger.info(f"Loaded ETF data: {df.shape} from {df.index.min()} to {df.index.max()}")
        ...
        if institute_files:
            ...
            df = self.normalize_timezone(df)
            data['institute'] = df
            logger.info(f"Loaded Silver Institute data: {df.shape} from {df.index.min()} to {df.index.max()}")
        
        return data
    
    def align_dates(self, df, other_df, how='inner'):
        """Align two dataframes on their indices"""
        common_dates = df.index.intersection(other_df.index)
        if len(common_dates) == 0:
            logger.warning(f"No common dates found! First index: {df.index[0]} to {df.index[-1]}, Other: {other_df.index[0]} to {other_df.index[-1]}")
            return df, other_df
        return df.loc[common_dates], other_df.loc[common_dates]
    ...
    def add_cross_asset_features(self, df, data):
        ...
        if 'gold' in data:
            ...
            df, gold = self.align_dates(df, gold)
            df['gold_close'] = gold
            ...
            # Correlation
            corr_window = 21
            df['gold_silver_corr'] = (df['returns_1d'].rolling(corr_window, min_periods=10)
                                       .corr(df['gold_close'].pct_change()))
        
        if 'market' in data:
            market = data['market']
            df, market = self.align_dates(df, market)
            ...
            for asset in ['vix', 'oil', 'copper', 'sp500']:
                ...
                if col in df.columns:
                    df[f'{asset}_corr'] = (df['returns_1d'].rolling(21, min_periods=10)
    ...
    def add_macro_features(self, df, data):
        ...
        if 'macro' in data:
            macro = data['macro']
            df, macro = self.align_dates(df, macro)
    ...
    def add_sentiment_features(self, df, data):
        ...
        if 'cot' in data:
            ...
            df, cot = self.align_dates(df, cot)
        ...
        if 'etf' in data:
            etf = data['etf']
            df, etf = self.align_dates(df, etf)
    ...
    def add_fundamental_features(self, df, data):
        ...
        if 'institute' in data:
            ...
            df, institute = self.align_dates(df, institute)
            ...
            if 'fund_stock_to_flow' in df.columns:
                df['fund_stock_to_flow_zscore'] = ((df['fund_stock_to_flow'] - 
                                                     df['fund_stock_to_flow'].rolling(252, min_periods=20).mean()) / 
                                                    df['fund_stock_to_flow'].rolling(252, min_periods=20).std())
        ...
        for col in columns:
            if col in df.columns:
                for window in windows:
                    for agg in aggs:
                        if agg == 'mean':
                            df[f'{col}_rolling_{window}_mean'] = df[col].rolling(window, min_periods=1).mean()
                        elif agg == 'std':
                            df[f'{col}_rolling_{window}_std'] = df[col].rolling(window, min_periods=1).std()
                        elif agg == 'min':
                            df[f'{col}_rolling_{window}_min'] = df[col].rolling(window, min_periods=1).min()
                        elif agg == 'max':
                            df[f'{col}_rolling_{window}_max'] = df[col].rolling(window, min_periods=1).max()
    ...
    def build_feature_matrix(self):
        ...
        logger.info(f"Silver date range: {df.index.min()} to {df.index.max()}")
...
def main():
    ...
    if df is not None:
        ...
        logger.error("Feature engineering failed - no feature matrix created")
