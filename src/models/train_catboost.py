#!/usr/bin/env python
"""
CatBoost Model Training with Walk-Forward Validation
Excellent with categorical features, less tuning needed
"""
...
class CatBoostTrainer:
    """Train CatBoost model with walk-forward validation"""
    ...
    def train(self, X_train, y_train, X_test, y_test, n_trials=15):
        """Train model with hyperparameter optimization"""
        
        logger.info("\n" + "="*50)
        logger.info("OPTUNA HYPERPARAMETER OPTIMIZATION")
        logger.info("="*50)
        logger.info("CatBoost requires fewer tuning iterations")
    ...
    def save_model(self, metrics):
        ...
        model_file = os.path.join(self.models_path, f"catboost_{timestamp}.cbm")
        ...
        results_file = os.path.join(self.results_path, f"catboost_results_{timestamp}.json")
        with open(results_file, 'w') as f:
            json.dump(results, f, indent=2, default=str)
        logger.info(f"📊 Results saved: {results_file}")
        
        imp_file = os.path.join(self.results_path, f"catboost_importance_{timestamp}.csv")
    ...
    def print_summary(self, metrics):
        ...
        logger.info("\n" + "="*60)
        logger.info("CATBOOST MODEL RESULTS")
        logger.info("="*60)
        ...
        if abs(metrics['train_rmse'] - metrics['test_rmse']) < 0.005:
            logger.info(f"\n✅ Model shows good generalization (minimal overfitting)")
        else:
            logger.info(f"\n⚠️ Model shows potential overfitting (train/test gap: {abs(metrics['train_rmse'] - metrics['test_rmse']):.4f})")
    
    def run(self):
        """Run the complete training pipeline"""
        
        logger.info("="*60)
        logger.info("CATBOOST MODEL TRAINING")
        logger.info("="*60)
        ...
        metrics = self.train(X_train, y_train, X_test, y_test, n_trials=15)
...
if __name__ == "__main__":
    ...
    trainer = CatBoostTrainer(target_col='target_next_day_return')
    trainer.run()
