...
class XGBoostTrainer:
    ...
    def load_data(self):
        ...
        # Look for final feature matrix (with VIF selection)
        final_files = list(Path(self.processed_path).glob("feature_matrix_final_*.csv"))
        
        if not final_files:
            # Fall back to reduced matrix
    ...
    def prepare_data(self, df, test_size=0.2):
        ...
        logger.info(f"After dropping NaN targets: {X.shape}")
    ...
    def objective(self, trial, X_train, y_train):
        ...
        for train_idx, val_idx in tscv.split(X_train):
            ...
            model = xgb.XGBRegressor(**params, early_stopping_rounds=10, verbose=False)
            model.fit(
                X_tr, y_tr,
    ...
    def train(self, X_train, y_train, X_test, y_test, n_trials=30):
    ...
    def save_model(self, metrics):
        ...
        # Save model
        model_file = os.path.join(self.models_path, f"xgboost_{timestamp}.pkl")
        ...
        results_file = os.path.join(self.results_path, f"xgboost_results_{timestamp}.json")
        with open(results_file, 'w') as f:
            json.dump(results, f, indent=2, default=str)
        logger.info(f"📊 Results saved: {results_file}")
        
        # Save feature importance CSV
        imp_file = os.path.join(self.results_path, f"xgboost_importance_{timestamp}.csv")
    ...
    def run(self):
        ...
        # Train model
        metrics = self.train(X_train, y_train, X_test, y_test, n_trials=30)
...
if __name__ == "__main__":
    trainer = XGBoostTrainer(target_col='target_next_day_return')
    trainer.run()
